...
function getData($datas)
{
  ...
  for ($i = 0; $tmpdata = xtc_db_fetch_array($tmpresult); $i++) {
    $opt_vals[$tmpdata['options_values_id']] = array(
      'options_values_price' => $tmpdata['options_values_price'],
      ...
      if ($search == $datas) {
        ...
        $JSON_ATTRDATA[$val_ids[$data_size]] = json_encode(
          [
            ...
            'vpevalue'     => (($vpe_status && (float)$vpe_value) ? (float)$vpe_value : 0),
            'attrvpevalue' => (($vpe_status && (float)$attr_vpe_value) ? (float)$attr_vpe_value : 0),
